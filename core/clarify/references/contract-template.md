...
<!-- 这个 Feature "做完了"的标准。每条必须可被程序验证。 -->
<!-- /goal 的完成条件由 compile-goal.mjs 从本节【确定性】推导（非 LLM 判断），因此： -->
<!--   · 验证方式必须从 4 类枚举里选，写成「验证方式: 编译|测试|脚本|命令」，不可自由文本 -->
<!--   · 每条 AC 必须标优先级「优先级: P0|P1」——P0 是核心（必须达成 goal），P1 是次要（失败转 ⚠️ 留 checkpoint） -->
<!--   · 验证方式为「测试」的 AC，必须在下一节「AC-测试映射」登记它对应的测试类，否则 verify-harness 的 SIG-ACCOV 会判未达成 -->
<!-- 4 类验证方式： -->
<!--   1. 编译 — 编译通过即满足（如"新增 XxxFacade 接口"） -->
<!--   2. 测试 — 单元测试断言验证（如"重复创建返回 XXX_DUPLICATE"） -->
<!--   3. 脚本 — verify-field-sync 等已安装脚本检查 -->
<!--   4. 命令 — 可执行 shell 命令判定（如 curl 返回 200） -->
<!-- 不可接受："用户体验好"、"代码清晰"、"性能足够"——无法程序验证的从本节移除，留给 checkpoint 人工确认 -->

- [ ] AC-001: {验收条件，关联 BR-001} — 验证方式: 测试 — 优先级: P0
- [ ] AC-002: {验收条件，关联 BR-002} — 验证方式: 测试 — 优先级: P0
- [ ] AC-003: {验收条件，关联状态机转换} — 验证方式: 测试 — 优先级: P0
- [ ] AC-004: {验收条件} — 验证方式: 脚本 — 优先级: P0
- [ ] AC-005: {可选的次要验收条件} — 验证方式: 命令 — 优先级: P1

## AC-测试映射

<!-- 验证方式为「测试」的 AC 必须在此登记对应测试，verify-harness 据此核验 SIG-ACCOV。 -->
<!-- 这张表是"测试作为 spec 的展开"——测试是 AC 的编译产物，不是事后补的验证。 -->
<!-- implement Phase 3 按本表逐条生成测试；verify-harness 按本表逐条核验存在性。 -->
<!-- 缺本节的契约不算 approved（clarify 守卫）。 -->

| AC | 测试类 | 测试方法/case | 断言类型 |
|----|--------|--------------|---------|
| AC-001 | {Domain}FacadeTest | case01/case02 | 正常 + 参数校验 |
| AC-002 | {Domain}FacadeTest | case03 | 业务错误（XXX_DUPLICATE） |
| AC-003 | {Domain}StateMachineTest | testInitToNext | 状态转换 |
| AC-005 | {Domain}FacadeIT | case01 | 集成命令验证 |
