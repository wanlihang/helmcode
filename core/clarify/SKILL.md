...
11. 写 AC-测试映射表（所有「验证方式: 测试」的 AC 必须登记对应测试类与方法）
...
**AC 结构化约束（compile-goal.mjs 机器解析的前提，必须遵守）**：

1. **验证方式枚举**：AC 的 `验证方式:` 字段必须从 `编译 / 测试 / 脚本 / 命令` 四选一，
   不得写自由文本（如"测试断言"→写"测试"；"verify-field-sync 脚本"→写"脚本"）。
   理由：`compile-goal.mjs` 按验证方式分类 AC 来确定性生成 goal，自由文本会破坏机器解析。
2. **优先级必标**：每条 AC 必须带 `优先级: P0 | P1`。
   - **P0（核心）**：必须达成 goal；失败则 goal 不达成。
   - **P1（次要）**：失败转判断日志 ⚠️，不阻塞 goal，留 /checkpoint 处理。
   默认全部 P0；只有「锦上添花、可延后」的 AC 才标 P1。
3. **AC-测试映射表必填**：所有「验证方式: 测试」的 AC，必须在契约「AC-测试映射」节
   登记对应测试类与方法。缺该节的契约不算 `approved`。映射表是「测试作为 spec 的展开」——
   测试是 AC 的编译产物，不是事后补的验证（借鉴 Spec Kit 测试先行的契约思想）。
4. **核心 AC 数量建议**：P0 AC 控制在 ≤5 条，避免 goal 全有或无卡死；复杂的拆 Feature。
