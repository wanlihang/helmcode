...
| Demo 业务参考 | `core/init-java-ddd/templates/app/*/blacklist/` + `templates/app/test/{BlacklistDemoSandbox,acts/blacklist/}` | Phase 7 按需 | 28 文件(21 业务文件[含 1 Mapper.xml] + 1 web Controller + 1 联调沙箱 + 1 ACTS 测试类 + 4 caseObjs.yaml) |
...
| `--with-web` | ❌ | `false` | 是否生成 `app/web` 模块;启用则含 REST Controller + 安全过滤 stub |
...
### Phase 3: 模块 pom.xml(5 核心 + 1 必选测试 + web 可选) + web 模块强制性资源(5 个,仅 `--with-web=true`)
...
```
app/application/src/main/java/{{basePackagePath}}/application/decider/.gitkeep
app/application/src/main/java/{{basePackagePath}}/application/shared/handler/.gitkeep
app/domain/src/main/java/{{basePackagePath}}/domain/.gitkeep
app/infrastructure/src/main/java/{{basePackagePath}}/infrastructure/config/.gitkeep
app/infrastructure/src/main/java/{{basePackagePath}}/infrastructure/log/.gitkeep
app/infrastructure/src/main/java/{{basePackagePath}}/infrastructure/messaging/.gitkeep
app/infrastructure/src/main/resources/mapper/.gitkeep
app/facade/src/main/java/{{basePackagePath}}/facade/.gitkeep
```

若 `--with-web=true`:额外创建 `app/web/src/main/java/{{basePackagePath}}/web/.gitkeep`。

> **分包策略**:按业务上下文({context})分包,不做按技术关注(facade/service/handler/convert)的水平分包。
> 业务代码一落地就进入 `<module>.<context>.<x>` 包,如 `application.mapping.handler.SavePdMappingHandler`。
> 骨架只预创建**跨功能点共享**的目录(`decider/`, `shared/handler/`, `config/`, `log/`, `messaging/`);
> 业务专属子包由 Demo 切片(Phase 7)和后续开发自然创建,无需 `.gitkeep` 占位。
>
> `application/decider/` — 签约模式决策等跨功能点共享的路由/决策逻辑。
> `application/shared/handler/` — HandlerTemplate 基类、Action 接口、OperateLogAction 等横切基础设施。
> `infrastructure/config/` — @Configuration 全局配置(Zdal/Mybatis/Sequence 等),不属于任何单一功能点。
> `infrastructure/log/` — MycmLoggerDef 等全局日志常量。
> `infrastructure/messaging/` — 领域事件发布等全局消息基础设施。
>
> `domain/` 和 `facade/` 只创建顶层 `.gitkeep`,不做 `<module>.model/` / `<module>.repository/` 等
> 子目录占位——由 Demo 切片和业务代码按 `<module>.<context>.<x>` 自然展开。
> 参考 `package-structure.md` 获取完整包路径约定。
...
| `templates/sql/V000__init_sequence.sql.tmpl` | `sql/V000__init_sequence.sql`(infra 级,**不属于 demo**,删 demo 时保留;含 `{{appName}}` 占位,需变量替换) |
...
从 `templates/app/<module>/blacklist/` 拷贝 21 个业务文件
(facade 8 + application 3 + domain 5 + infrastructure 5,其中 infra 含 1 个 `*Mapper.xml`);
若 `--with-web=true`,再加 1 个 `BlacklistController.java`;
若 `--with-test=true`,再从 `templates/app/test/` 额外拷贝 6 个测试 Demo 文件
(1 个手工联调沙箱 + 1 个 ACTS 测试类 + 4 个 caseObjs.yaml: case01~case04)。
默认(`--with-web=true`+`--with-test=true`)共 28 个 Demo 文件,
仅核心 5 模块(无 web/test)时 21 个。**每个 Demo 文件头部必须保留**
...
>   纯联调脚手架。
> - ACTS 集成测试 `CheckEmailActsTest.java` + `case01~case04 caseObjs.yaml`:STANDARD 主推方式,
...
>   case 覆盖:case01 正常路径、case02 参数校验、case03 依赖失败、case04 业务错误。

切片业务:邮箱黑名单管理,5 个 facade 方法(按 Manage/Query 分离模式):
- `Result<Boolean> checkEmail(BlacklistQueryRequest)` — 读(BlacklistQueryFacade)
- `Result<Paginator<BlacklistVO>> listByPage(BlacklistQueryRequest)` — 分页查询(BlacklistQueryFacade)
- `Result<Void> addBlacklist(@Valid BlacklistAddCommand)` — 写,带幂等(BlacklistManageFacade)
- `Result<Void> removeBlacklist(@Valid BlacklistRemoveCommand)` — 软删(BlacklistManageFacade)
- `Result<Void> batchAddBlacklist(@Valid BlacklistBatchAddCommand)` — 批量写,用 `TransactionTemplate`(BlacklistManageFacade)

涉及文件目录(垂直切片,统一在 `.blacklist.` 包下):
```
app/facade/src/main/java/{{basePackagePath}}/facade/blacklist/BlacklistManageFacade.java
app/facade/src/main/java/{{basePackagePath}}/facade/blacklist/BlacklistQueryFacade.java
app/facade/src/main/java/{{basePackagePath}}/facade/blacklist/model/command/BlacklistAddCommand.java
app/facade/src/main/java/{{basePackagePath}}/facade/blacklist/model/command/BlacklistRemoveCommand.java
app/facade/src/main/java/{{basePackagePath}}/facade/blacklist/model/command/BlacklistBatchAddCommand.java
app/facade/src/main/java/{{basePackagePath}}/facade/blacklist/model/query/BlacklistCheckRequest.java
app/facade/src/main/java/{{basePackagePath}}/facade/blacklist/model/query/BlacklistQueryRequest.java
app/facade/src/main/java/{{basePackagePath}}/facade/blacklist/model/vo/BlacklistVO.java

app/application/src/main/java/{{basePackagePath}}/application/blacklist/facade/BlacklistManageFacadeImpl.java
app/application/src/main/java/{{basePackagePath}}/application/blacklist/facade/BlacklistQueryFacadeImpl.java
...
app/test/src/test/java/{{basePackagePath}}/acts/test/blacklist/checkEmail/case02_invalid_email/caseObjs.yaml                    ← 若 --with-test
app/test/src/test/java/{{basePackagePath}}/acts/test/blacklist/checkEmail/case03_dependency_fail/caseObjs.yaml                  ← 若 --with-test
app/test/src/test/java/{{basePackagePath}}/acts/test/blacklist/checkEmail/case04_biz_error/caseObjs.yaml                        ← 若 --with-test
...
> - `templates/app/test/acts/blacklist/checkEmail/case01_success/caseObjs.yaml.tmpl` —— 正常路径
> - `templates/app/test/acts/blacklist/checkEmail/case02_invalid_email/caseObjs.yaml.tmpl` —— 参数校验
> - `templates/app/test/acts/blacklist/checkEmail/case03_dependency_fail/caseObjs.yaml.tmpl` —— 依赖失败
> - `templates/app/test/acts/blacklist/checkEmail/case04_biz_error/caseObjs.yaml.tmpl` —— 业务错误
>   ACTS 用例数据均为 7 段式(Case Desc / Arguments / Flags / Result / Message Event /
>   User-defined Params / Virtual Mocks),覆盖 test-standards.md 要求的 4 种必测场景。
>
...
- ✅ `--with-demo=true` 时:21 个 demo 业务文件存在(facade 8 + application 3 + domain 5 + infrastructure 5,含 `EmailBlacklistDOMapper.xml`);`--with-web=true` 再 +1(Controller);`--with-test=true` 再 +6(沙箱 + ACTS 测试类 + 4 caseObjs.yaml)。每个文件头有 `HelmCode Demo slice` marker
