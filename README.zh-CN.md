...
每个 turn 结束 `verify` 必须打印以下 4 项信号,全部通过 Haiku 评估器才判 `goal achieved`:

| 检查项 | 通过信号 | 失败处理 |
| --- | --- | --- |
| **编译** | `BUILD SUCCESS` | 下一 turn 分析错误并修复 |
...
> **`Tests run ≥ 1` 是 v2.1.0 新增的硬约束。** 防止 AI 跳过测试用 `Tests run: 0, Failures: 0` 骗 Haiku 误判 `goal achieved`。详见 `core/verify/SKILL.md` §2。
